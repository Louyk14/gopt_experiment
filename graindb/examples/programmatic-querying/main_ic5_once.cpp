...
int test_graindb_smergejoin_person_constrained(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
...
int test_graindb_smergejoin_person_constrained_large(string constantval, int& rnum, Connection& con) {
	...
	join_condition_cof_forum.left = make_unique<BoundReferenceExpression>("forumid", TypeId::INT64, 0);
...
int test_graindb_smergejoin_ldbc(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
...
int test_graindb_smergejoin_ldbc_large(string constantval, int& rnum, Connection& con) {
	...
	vector<TypeId> output_path_types_2{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR,
	                                   TypeId::VARCHAR, TypeId::INT64, TypeId::INT64, TypeId::VARCHAR};
	join_path_2_op.types = output_path_types_2;
...
int main() {
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r0num = 0;
			duck_time += test_duckdb_ldbc(constantval, r0num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r1num = 0;
			grain_ldbc_time += test_graindb_smergejoin_ldbc(constantval, r1num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r4num = 0;
			grain_ldbc_large_time += test_graindb_smergejoin_ldbc_large(constantval, r4num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r2num = 0;
			grain_time += test_graindb_smergejoin_person_constrained(constantval, r2num, con);
	...
	std::cout << "GRain LDBC Large Avg Time Cost: " << (double) grain_ldbc_large_time / constantval_list.size() << std::endl;
