...
// graindb style query
int test_graindb_smergejoin_person_constrained_large(string constantval, int& rnum, Connection& con) {
	...
	join_condition_cof.left = make_unique<BoundReferenceExpression>("postid_rowid", TypeId::INT64, 0);
	...
	vector<TypeId> output_cof_types{TypeId::INT64, TypeId::INT64, TypeId::INT64, TypeId::INT64};
	...
	vector<JoinCondition> cond_cof_forum;
	JoinCondition join_condition_cof_forum;
	join_condition_cof_forum.left = make_unique<BoundReferenceExpression>("forumid_rowid", TypeId::INT64, 1);
	join_condition_cof_forum.right = make_unique<BoundReferenceExpression>("forum_rowid", TypeId::INT64, 2);
	join_condition_cof_forum.comparison = ExpressionType::COMPARE_EQUAL;

	auto rai_info_cof_forum = make_unique<RAIInfo>();
	rai_info_cof_forum->rai = table_containerof->storage->info->rais[0].get();  // 3640
	rai_info_cof_forum->rai_type = RAIType::EDGE_SOURCE;
	rai_info_cof_forum->forward = true;
	rai_info_cof_forum->vertex = table_forum;
	rai_info_cof_forum->vertex_id = table_index_forum;
	rai_info_cof_forum->passing_tables[0] = table_index_containerof;
	rai_info_cof_forum->left_cardinalities[0] = table_containerof->storage->info->cardinality;
	rai_info_cof_forum->compact_list = &rai_info_cof_forum->rai->alist->compact_forward_list; //e498

	join_condition_cof_forum.rais.push_back(move(rai_info_cof_forum));
	cond_cof_forum.push_back(move(join_condition_cof_forum));

	LogicalComparisonJoin join_cof_forum_op(JoinType::INNER);
	vector<TypeId> output_types_cof_forum{TypeId::INT64, TypeId::INT64, TypeId::INT64, TypeId::INT64,
	                                      TypeId::VARCHAR, TypeId::VARCHAR, TypeId::INT64};
	join_cof_forum_op.types = output_types_cof_forum;
	auto join_cof_forum = make_unique<PhysicalSIPJoin>(context, join_cof_forum_op, move(join_cof),
	                                                         move(scan_function_forum), move(cond_cof_forum), join_cof_forum_op.join_type,
	                                                         left_projection_map, right_projection_map);
	...
	join_condition_person2.left = make_unique<BoundReferenceExpression>("id2_rowid", TypeId::INT64, 1);
	join_condition_person2.right = make_unique<BoundReferenceExpression>("person_rowid", TypeId::INT64, 0);
	...
	rai_info_person2->rai_type = RAIType::EDGE_TARGET;
	rai_info_person2->forward = false;
	rai_info_person2->vertex = table_person;
	rai_info_person2->vertex_id = table_index_person2;
	rai_info_person2->passing_tables[0] = table_index_knows;
	rai_info_person2->left_cardinalities[0] = table_knows->storage->info->cardinality;
	rai_info_person2->compact_list = &rai_info_person2->rai->alist->compact_backward_list;
	...
	vector<TypeId> output_person2_types{TypeId::VARCHAR, TypeId::INT64, TypeId::INT64};
	join_person2_op.types = output_person2_types;
	auto join_person2 = make_unique<PhysicalSIPJoin>(context, join_person2_op, move(scan_function_knows),
	                                                 move(scan_function_person2), move(cond_person2), join_person2_op.join_type,
	                                                 left_projection_map, right_projection_map);

	// left join right
	vector<JoinCondition> cond_left_right;
	JoinCondition join_condition_left_right;
	join_condition_left_right.left = make_unique<BoundReferenceExpression>("personid_rowid", TypeId::INT64, 3);
	join_condition_left_right.right = make_unique<BoundReferenceExpression>("person_rowid", TypeId::INT64, 2);
	join_condition_left_right.comparison = ExpressionType::COMPARE_EQUAL;

	auto rai_info_left_right = make_unique<RAIInfo>();
	rai_info_left_right->rai = table_hascreator->storage->info->rais[0].get(); // 2d40
	rai_info_left_right->rai_type = RAIType::EDGE_TARGET;
	rai_info_left_right->forward = false;
	rai_info_left_right->vertex = table_person;
	rai_info_left_right->vertex_id = table_index_person2;
	rai_info_left_right->passing_tables[0] = table_index_hascreator;
	rai_info_left_right->left_cardinalities[0] = table_hascreator->storage->info->cardinality;
	rai_info_left_right->compact_list = &rai_info_left_right->rai->alist->compact_backward_list;

	join_condition_left_right.rais.push_back(move(rai_info_left_right));

	cond_left_right.push_back(move(join_condition_left_right));

	LogicalComparisonJoin join_left_right_op(JoinType::INNER);
	vector<TypeId> output_left_right_types{TypeId::INT64, TypeId::INT64, TypeId::INT64, TypeId::INT64,
	                                       TypeId::VARCHAR, TypeId::VARCHAR, TypeId::INT64,
	                                       TypeId::INT64};
	join_left_right_op.types = output_left_right_types;
	vector<idx_t> right_projection_map_left_right{2};
	auto join_left_right = make_unique<PhysicalSIPJoin>(context, join_left_right_op, move(join_cof_forum),
	                                                           move(join_person2), move(cond_left_right), join_left_right_op.join_type,
	                                                           left_projection_map, right_projection_map_left_right);

	// join hasmember
	vector<idx_t> hasmember_ids{0, 3};
	...
	join_condition_hasmember1.right = make_unique<BoundReferenceExpression>("person_rowid", TypeId::INT64, 7);
	join_condition_hasmember1.comparison = ExpressionType::COMPARE_EQUAL;

	auto rai_info_hasmember = make_unique<RAIInfo>();
	rai_info_hasmember->rai = table_hasmember->storage->info->rais[0].get(); // 2d40
	rai_info_hasmember->rai_type = RAIType::EDGE_TARGET;
	rai_info_hasmember->forward = false;
	rai_info_hasmember->vertex = table_person;
	rai_info_hasmember->vertex_id = table_index_person2;
	...
	rai_info_hasmember->compact_list = &rai_info_hasmember->rai->alist->compact_backward_list;

	join_condition_hasmember1.rais.push_back(move(rai_info_hasmember));

	join_condition_hasmember2.left = make_unique<BoundReferenceExpression>("forumid", TypeId::VARCHAR, 0);
	join_condition_hasmember2.right = make_unique<BoundReferenceExpression>("id", TypeId::VARCHAR, 4);
	...
	vector<TypeId> output_hasmember{TypeId::VARCHAR, TypeId::INT64, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::INT64};
	join_hasmember_op.types = output_hasmember;
	vector<idx_t> right_projection_map_hasmember{4, 5, 7};
	auto join_hasmember = make_unique<PhysicalSIPJoin>(context, join_hasmember_op, move(scan_function_hasmember),
	                                                move(join_left_right), move(cond_hasmember), join_hasmember_op.join_type,
...
// postgres style query
int test_graindb_smergejoin_ldbc_large(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
	...
	join_condition_path_21.left = make_unique<BoundReferenceExpression>("f_forumid", TypeId::VARCHAR, 1);
	join_condition_path_21.right = make_unique<BoundReferenceExpression>("forumid", TypeId::VARCHAR, 3);
	join_condition_path_21.comparison = ExpressionType::COMPARE_EQUAL;

	join_condition_path_22.left = make_unique<BoundReferenceExpression>("k_person2id", TypeId::VARCHAR, 2);
	join_condition_path_22.right = make_unique<BoundReferenceExpression>("personid", TypeId::VARCHAR, 0);
	...
	vector<TypeId> output_path_types_2{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR,
	                                   TypeId::VARCHAR, TypeId::VARCHAR};
	join_path_2_op.types = output_path_types_2;
	vector<idx_t> right_projection_map_join_path_2{0, 3};
	auto join_path_2 = make_unique<PhysicalHashJoin>(context, join_path_2_op, move(inner_projection),
	                                                 move(join_hascreator), move(cond_path_2), join_path_2_op.join_type,
	                                                 left_projection_map, right_projection_map_join_path_2);

	// project
	vector<TypeId> result_types{TypeId::VARCHAR};
	vector<unique_ptr<Expression>> select_list;
	auto result_col0 = make_unique<BoundReferenceExpression>("title", TypeId::VARCHAR, 0);
	select_list.push_back(move(result_col0));
	auto projection = make_unique<PhysicalProjection>(result_types, move(select_list));
...
// graindb style query
int test_duckdb_constrained(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
	...
	LogicalComparisonJoin join_hascreator_op(JoinType::INNER);
	vector<TypeId> output_hascreator{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_hascreator_op.types = output_hascreator;
	vector<idx_t> right_projection_map_hascreator{0};
	...
	LogicalComparisonJoin join_cof_op(JoinType::INNER);
	vector<TypeId> output_cof{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_cof_op.types = output_cof;
	vector<idx_t> right_projection_map_cof{0, 1};
	...
	join_condition_post1.right = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 1);
	...
	join_condition_post2.right = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 1);
	...
	auto join_post = make_unique<PhysicalHashJoin>(context, join_post_op, move(scan_function_post),
	                                               move(join_cof), move(cond_post), join_post_op.join_type,
	...
	join_condition_forum.right = make_unique<BoundReferenceExpression>("forumid", TypeId::VARCHAR, 1);
	join_condition_forum.comparison = ExpressionType::COMPARE_EQUAL;
	...
	vector<idx_t> right_projection_map_forum{1, 3};
	...
	join_condition_hasmember2.right = make_unique<BoundReferenceExpression>("forumid", TypeId::VARCHAR, 0);
	join_condition_hasmember2.comparison = ExpressionType::COMPARE_EQUAL;
	...
	LogicalComparisonJoin join_hasmember_op(JoinType::INNER);
	vector<TypeId> output_hasmember{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_hasmember_op.types = output_hasmember;
...
// postgres style query
int test_duckdb_ldbc(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
	...
	vector<idx_t> hasmember_ids{0, 1};
	vector<TypeId> get_hasmember_types{TypeId::VARCHAR, TypeId::VARCHAR};
	auto scan_function_hasmember = move(get_scan_function(context, table_edge_hasmember,
	                                                      table_index_hasmember, hasmember_ids, get_hasmember_types));
	...
	vector<JoinCondition> cond_hasmember_knows;
	JoinCondition join_condition_hasmember_knows;
	join_condition_hasmember_knows.left = make_unique<BoundReferenceExpression>("personid", TypeId::VARCHAR, 1);
	join_condition_hasmember_knows.right = make_unique<BoundReferenceExpression>("id2", TypeId::VARCHAR, 0);
	join_condition_hasmember_knows.comparison = ExpressionType::COMPARE_EQUAL;

	cond_hasmember_knows.push_back(move(join_condition_hasmember_knows));

	LogicalComparisonJoin join_hasmember_knows_op(JoinType::INNER);
	vector<TypeId> output_hasmember_knows{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_hasmember_knows_op.types = output_hasmember_knows;
	vector<idx_t> right_projection_map_hasmember_knows_map{0};
	auto join_hasmember_knows = make_unique<PhysicalHashJoin>(context, join_hasmember_knows_op, move(scan_function_hasmember),
	                                                        move(scan_function_knows), move(cond_hasmember_knows), join_hasmember_knows_op.join_type,
	                                                        left_projection_map, right_projection_map_hasmember_knows_map);
	...
	vector<TypeId> output_forum{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_forum_op.types = output_forum;
	vector<idx_t> right_projection_map_forum{0, 1};
	auto join_forum = make_unique<PhysicalHashJoin>(context, join_forum_op, move(scan_function_forum),
	                                                move(join_hasmember_knows), move(cond_forum), join_forum_op.join_type,
	                                                left_projection_map, right_projection_map_forum);

	// project forum
	vector<TypeId> result_types_forum{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	vector<unique_ptr<Expression>> select_list_forum;
	auto result_forum_col0 = make_unique<BoundReferenceExpression>("title", TypeId::VARCHAR, 1);
	auto result_forum_col1 = make_unique<BoundReferenceExpression>("f_forumid", TypeId::VARCHAR, 2);
	auto result_forum_col2 = make_unique<BoundReferenceExpression>("k_person2id", TypeId::VARCHAR, 3);
	select_list_forum.push_back(move(result_forum_col0));
	select_list_forum.push_back(move(result_forum_col1));
	select_list_forum.push_back(move(result_forum_col2));
	auto projection_forum = make_unique<PhysicalProjection>(result_types_forum, move(select_list_forum));
	projection_forum->children.push_back(move(join_forum));
	...
	vector<JoinCondition> cond_post_cof;
	JoinCondition join_condition_post_cof;
	join_condition_post_cof.left = make_unique<BoundReferenceExpression>("id", TypeId::VARCHAR, 0);
	join_condition_post_cof.right = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 1);
	join_condition_post_cof.comparison = ExpressionType::COMPARE_EQUAL;

	cond_post_cof.push_back(move(join_condition_post_cof));

	LogicalComparisonJoin join_post_cof_op(JoinType::INNER);
	vector<TypeId> output_post_cof{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_post_cof_op.types = output_post_cof;
	auto join_post_cof = make_unique<PhysicalHashJoin>(context, join_post_cof_op, move(scan_function_post),
	                                              move(scan_function_cof), move(cond_post_cof), join_post_cof_op.join_type,
	                                              left_projection_map, right_projection_map);
	...
	vector<JoinCondition> cond_hascreator;
	JoinCondition join_condition_hascreator1, join_condition_hascreator2;
	join_condition_hascreator1.left = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 0);
	join_condition_hascreator1.right = make_unique<BoundReferenceExpression>("id", TypeId::VARCHAR, 0);
	join_condition_hascreator1.comparison = ExpressionType::COMPARE_EQUAL;

	join_condition_hascreator2.left = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 0);
	join_condition_hascreator2.right = make_unique<BoundReferenceExpression>("postid", TypeId::VARCHAR, 0);
	join_condition_hascreator2.comparison = ExpressionType::COMPARE_EQUAL;
	...
	vector<JoinCondition> cond_left_right;
	JoinCondition join_condition_left_right1, join_condition_left_right2;
	join_condition_left_right1.left = make_unique<BoundReferenceExpression>("k_person2id", TypeId::VARCHAR, 2);
	join_condition_left_right1.right = make_unique<BoundReferenceExpression>("personid", TypeId::VARCHAR, 1);
	join_condition_left_right1.comparison = ExpressionType::COMPARE_EQUAL;

	join_condition_left_right2.left = make_unique<BoundReferenceExpression>("f_forumid", TypeId::VARCHAR, 1);
	join_condition_left_right2.right = make_unique<BoundReferenceExpression>("forumid", TypeId::VARCHAR, 3);
	join_condition_left_right2.comparison = ExpressionType::COMPARE_EQUAL;

	cond_left_right.push_back(move(join_condition_left_right1));
	cond_left_right.push_back(move(join_condition_left_right2));

	LogicalComparisonJoin join_left_right_op(JoinType::INNER);
	vector<TypeId> output_left_right{TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR, TypeId::VARCHAR};
	join_left_right_op.types = output_left_right;
	vector<idx_t> right_projection_map_left_right{1, 3};
	auto join_left_right = make_unique<PhysicalHashJoin>(context, join_left_right_op, move(projection_forum),
	                                                    move(join_hascreator), move(cond_left_right), join_left_right_op.join_type,
	                                                    left_projection_map, right_projection_map_left_right);
	...
	select_list.push_back(move(result_col0));
	auto projection = make_unique<PhysicalProjection>(result_types, move(select_list));
	projection->children.push_back(move(join_left_right));
...
int test_gopt_join(string constantval, int& rnum, Connection& con) {
	auto &context = *con.context;
...
int main() {
	...
	int duck_ldbc_time = 0;
	int duck_constrained_time = 0;
	int grain_ldbc_time = 0;
	int grain_constrained_time = 0;
	int gopt_time = 0;

	vector<vector<int>> results(5, vector<int>());

	{
		DuckDB* db = new DuckDB(nullptr);
		Connection con(*db);
		...
		for (const auto &constantval : constantval_list) {
			...
			std::cout << "DUCK PLAN WITH LDBC: " << std::endl;
			int r0num = 0;
			duck_ldbc_time += test_duckdb_ldbc(constantval, r0num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			std::cout << "Testing " << constantval << " " << counter++ << "/" << count_num << std::endl;
			std::cout << "DUCK PLAN WITH CONSTRAINED: " << std::endl;
			int r1num = 0;
			duck_constrained_time += test_duckdb_constrained(constantval, r1num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			std::cout << "Testing " << constantval << " " << counter++ << "/" << count_num << std::endl;
			std::cout << "GRAIN PLAN WITH LARGE LDBC QUERY: " << std::endl;
			int r4num = 0;
			grain_ldbc_time += test_graindb_smergejoin_ldbc_large(constantval, r4num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r2num = 0;
			grain_constrained_time += test_graindb_smergejoin_person_constrained_large(constantval, r2num, con);
...

	{
		...
		for (const auto &constantval : constantval_list) {
			...
			int r3num = 0;
			gopt_time += test_gopt_join(constantval, r3num, con);
	...
	/*for (int i = 0; i < results.size(); ++i) {
		for (int j = 0; j < constantval_list.size(); ++j) {
			std::cout << results[i][j] << " ";
		}
		std::cout << std::endl;
	}*/

	std::cout << "Duck LDBC Time Cost: " << (double) duck_ldbc_time / constantval_list.size() << std::endl;
	std::cout << "Duck Constrained Time Cost: " << (double) duck_constrained_time / constantval_list.size() << std::endl;
	std::cout << "GRain LDBC Time Cost: " << (double) grain_ldbc_time / constantval_list.size() << std::endl;
	std::cout << "GRain Constrained Large Avg Time Cost: " << (double) grain_constrained_time / constantval_list.size() << std::endl;
